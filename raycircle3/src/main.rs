...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    println!(" low common multiple{:?}", l);

    // Somma numeratori
    let mut somma_numeratori = 0;
    for n in 1..=parti {
        somma_numeratori += l / n;
    }
    println!("somma numeratori{:?}", somma_numeratori);

    let step = (distanza * l as f32) / somma_numeratori as f32;
    println!("step{:?}", step);

    let mut prev_y = 0.0;
    for n in 1..=parti {
        let diam = (1.0 / n as f32) * step;
        draw.ellipse()
            .x_y(0.0, 0.0 + prev_y)
            .w_h(diam, diam)
            .color(BLACK);
        prev_y += diam / 2.0 + ((1.0 / (n as f32 + 1.0)) * step) / 2.0;
    }
